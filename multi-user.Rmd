...
- Each participant 
  ...
  - we on purpose to not cover account creation, `root` use, ...
...
This runs as the user starting the sessione, _i.e._ the one granting access
to his session.
...
```sh
$ tmux -S /tmp/scratch/session attach-session -t joint 
```
